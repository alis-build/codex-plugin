...
- **`frontend-preview`** — shows a service's web frontend in [terminal-browser](https://terminal-browser.com), a real browser drawn in a terminal pane beside Codex, and drives it (`alis preview --json`, then `terminal-browser action` to snapshot, click, fill and read the console; `--ssh alis-<org>-<id>` for a workstation's dev server). Needs terminal-browser and a kitty-graphics terminal that can split (Ghostty, kitty, WezTerm, tmux or herdr); terminal-browser runs with escalated permissions. Never used for logs or operation status.
